...
{
public:
	Rectangle(Vector2 position, Vector2 size, RGB color, std::string name, MoveDirection direction) : Shape(ShapeType::RECTANGLE, position, color, name, direction), _size(size) {}
	~Rectangle() {}
	
#pragma region Getters
	float GetWidth() const { return _size.x; }
	float GetHeight() const { return _size.y; }
	Vector2 GetCenter() const { return GetPosition() + _size / 2; }
#pragma endregion
